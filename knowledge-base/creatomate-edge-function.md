# Creatomate Edge Function (Supabase)

This is the actual code for the Creatomate Edge Function deployed on our remote Supabase instance.
**Note:** This is for reference/documentation only. Do not run this file locally.

<!-- ../_shared/creatomate-api.ts -->
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.49.4';
import { CreatomateApi } from '../_shared/creatomate-api.ts';
...
async function getCreatomateApiKey(supabaseClient) {
  try {
    const { data, error } = await supabaseClient.from('secrets').select('value').eq('name', 'CREATOMATE_API_KEY').maybeSingle();
    if (error || !data) {
...
function extractVariablesFromCurl(text) {
  try {
...
async function getTemplateRenderInfo(templateId, creatomateApi) {
  try {
...
function extractVariablesFromElements(elements) {
  const variables = {};
...
const mapCreatomateStatus = (status)=>{
  const statusMap = {
    'planned': 'pending',
    'waiting': 'pending',
    'queued': 'pending',
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(JSON.stringify({
        error: 'API key not configured in secrets table'
      }), {
        status: 500,
        headers: corsHeaders
      });
    ...
    if (action === 'import-template') {
      ...
      try {
        ...
        if (templateData.outputs && Array.isArray(templateData.outputs)) {
          platforms = templateData.outputs.map((output)=>({
      ...
      const { creatomateTemplateId, template_id, variables, platforms, user_id, database_job_id } = requestBody;
 ...
 */ export class CreatomateApi {
   ...
   */ async createRender(params) {
    try {
      const data = await this.request('/renders', 'POST', params);
   ...
   */ async getRender(id) {
    try {
      const data = await this.request(`/renders/${id}`, 'GET');
      // Parse and validate the response
      return RenderSchema.parse(data);
    } catch (error) {
   ...
   */ async getTemplate(id) {
    try {
      const data = await this.request(`/templates/${id}`, 'GET');
      // Parse and validate the response
   ...
   */ async awaitRender(id, interval = 5000, maxRetries = 60) {
   ...
   */ handleError(error) {
    ...
    if (error.response) {
      ...
      return new Error(`Creatomate API error (${status}): ${typeof data === 'string' ? data : JSON.stringify(data)}`);
    }
